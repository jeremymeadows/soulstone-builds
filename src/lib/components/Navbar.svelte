<script lang="ts">
	...
	function toggle_menu() {
		...
		document.getElementById(menu.getAttribute('data-target')!)!.classList.toggle('is-active');
	}
	...
	$effect(() => {
		if (navigating) {
			toggle_menu();
			document.querySelector('.is-current-page')?.classList.remove('is-current-page');
			document
		...
		<div class="navbar-end">
			{#each Object.entries(pages) as [page, url]}
				<a
					href={url}
					class="navbar-item"
					data-sveltekit-preload-data={url === '/signout' ? 'off' : ''}
				>
					{page}
				</a>
			{/each}
